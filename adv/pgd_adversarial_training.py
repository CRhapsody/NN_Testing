...
def test(epoch):
    ...
    print('Model Saved!')
    with open('./checkpoint/' + file_name + '.txt', 'a') as f:
        f.write('Epoch: %d\n' % epoch)
        f.write('Total benign test accuarcy: %f\n' % (100. * benign_correct / total))
        f.write('Total adversarial test Accuarcy: %f\n' % (100. * adv_correct / total))
        f.write('Total benign test loss: %f\n' % benign_loss)
        f.write('Total adversarial test loss: %f\n' % adv_loss)
        f.write('\n')
